...
a)备份原driver.py,driver.pyc,driver.pyo文件
  ...
  如：mv /usr/lib/python2.7/site-packages/nova/virt/libvirt/driver.py /usr/lib/python2.7/site-packages/nova/virt/libvirt/driver.py_20171122
...
b)把usb_passthrough目录下driver.py 文件复制到目录： /usr/lib/python2.7/site-packages/nova/virt/libvirt/
  如：cp /opt/usb_passthrough/driver.py /usr/lib/python2.7/site-packages/nova/virt/libvirt/
...
4、查询USB加密狗信息(屏幕输出同时保存到usb_devices_list文件)
   执行：sh /opt/usb_passthrough/find_usb.sh
   如下输出：
   ####################################################
